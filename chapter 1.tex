...
First, $\tilde{M}$ will copy the input into the encoded section of the tape. The tape will have the configuration $\triangleright s_{in} \triangleright s_{enc} \triangleright$. To simulate each step of $M$, we sweep right and left 4 times.
\begin{itemize}
...
\end{itemize}
To make this ``oblivious," we expand the encoded section by exactly $k$ positions every cycle. This makes the head position of $\tilde{M}$ completely predictable.
...
\begin{cases}
(q, \textsf{READ2}, s, \textsf{inc}(i)), \square, \textsf{R} & \text{if } \tilde{\sigma} = \triangleright, i = 1 \\
...
\begin{cases}
(q, \textsf{WRITE}, \delta(q, s), \textsf{dec}(i)), \triangleright, \textsf{L} & \text{if } i = 1 \\
...
\begin{cases}
(q, \textsf{RSEEK}, q', s', d, \textsf{inc}(i)), \triangleright, \textsf{R} & \text{if } \tilde{\sigma} = \triangleright, i = k \\
...
\end{multline*}
$$
\tilde{\delta}((q, \textsf{RSEEK}, q', s', d, i), \tilde{\sigma}) = \,???
$$
TODO: Figure out how to deal with left shifts off the side of the tape (maybe put $\triangleright^k$ at the start).
